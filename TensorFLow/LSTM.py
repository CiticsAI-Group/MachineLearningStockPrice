    ...
    with tf.Session() as sess:
        ...
        for i in range(100):
            for step in range(len(batch_index)-1):
                _, acc_=sess.run([train_op,accuracy],feed_dict={X:train_x[batch_index[step]:batch_index[step+1]],Y:train_y[batch_index[step]:batch_index[step+1]],keep_prob: 0.8})
                #print("Iter%d, step %d, training accuracy %f" % (i,step, acc_,))
            if i % 5==0:
                #print("保存模型：",saver.save(sess,'test.model',global_step=i))
                train_accuracy,prediction,label = sess.run([accuracy,pred,Y], feed_dict={X:train_x[batch_index[step]:batch_index[step+1]],Y:train_y[batch_index[step]:batch_index[step+1]], keep_prob: 1.0})
                print ("Iter%d, step %d, training accuracy %f" % (i,step, train_accuracy))
