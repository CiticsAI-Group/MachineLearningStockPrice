...
class LSTMModel:
    ...
    def get_model(self):
        ...
        net = tflearn.lstm(net, int(self.hidden_layers*0.6),dropout=0.8, return_seq=True)
        net = tflearn.lstm(net, int(self.hidden_layers*0.6*0.6), dropout=0.8)
...
def train_model(CLASS,predict_day,FORCE=False):
    ...
    print(lstm_model.CHECK_POINT_PATH + Constant.CHECKPOINT)
    isNew = True
    if (os.path.isfile(lstm_model.CHECK_POINT_PATH + Constant.CHECKPOINT)):
        lstm_model.model.load(
            lstm_model.CHECK_POINT_PATH + 'tflearn.model.' + str(CLASS))
        isNew = False
    if (FORCE == True):
        isNew = True
    X = []
    ...
    lstm_model.model.save(lstm_model.CHECK_POINT_PATH + 'tflearn.model.' + str(CLASS))
